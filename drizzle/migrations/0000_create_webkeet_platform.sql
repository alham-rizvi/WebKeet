...
CREATE TABLE public.lab_authors (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  name text NOT NULL,
...
CREATE UNIQUE INDEX one_active_instance_per_user ON public.lab_instances(user_id) WHERE status IN ('provisioning', 'running');
...
CREATE INDEX labs_category_idx ON public.labs(category);
CREATE INDEX labs_difficulty_idx ON public.labs(difficulty);
CREATE INDEX completions_user_idx ON public.completions(user_id);
CREATE INDEX instances_user_status_idx ON public.lab_instances(user_id, status);
